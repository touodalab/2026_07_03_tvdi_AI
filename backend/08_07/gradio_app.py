...
# 0. 後端 API 連線設定
# ==========================================
# 此為「前端程式」，透過 HTTP 呼叫 FastAPI 後端 (backend/08_07/app.py)。
# 可透過環境變數 SALARY_API_BASE 覆寫後端位址。
...
# 1. RWD (響應式網頁設計) 全域樣式
# ==========================================
# 說明：此區塊為 RWD 架構核心。利用 CSS Grid + auto-fit、
# clamp() 流式字體與 media query 斷點，讓自訂 HTML 卡片
...
def make_prediction_card(pred: float, model_type: str, r2: float) -> str:
    """薪資預測結果卡片 (與 Iris 服務的預測卡片風格一致)"""
    monthly = pred * 10000 / 12
    return f"""
    <div class="rwd-card" style="background-color: #e6f4ea; color: #137333; border-color: rgba(19, 115, 51, 0.25);">
...
def make_coef_chart(feature_coefs: dict[str, float]) -> str:
    ...
    html = '<h4 style="margin:0 0 8px 0; font-size:1.1rem; font-weight:700; color:#202124;">⚖️ 特徵權重係數 (Feature Coefficients)</h4>'
    html += '<div class="rwd-bars">'
...
# ==========================================
# 5. 建立 Gradio UI (RWD + 風格仿 Iris 服務)
# ==========================================
with gr.Blocks(
    ...
    gr.Markdown(
        ...
        本系統展示機器學習模型部署的**完整生命週期**。此為 Gradio 前端，底層使用 **FastAPI** 驅動的 RESTful API 服務。
        * 🔮 **即時預測分頁**：輸入工作年資、學歷與城市，即時取得預測年薪與各特徵貢獻分析。
        * ⚙️ **線上訓練分頁**：可調整模型演算法與超參數，即時呼叫後端重新訓練並查看評估指標與特徵權重。
        """
    )
